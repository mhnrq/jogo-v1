...
function updateCounts() {
  ...
  document.getElementById(
    "quadraticaCount"
  ).textContent = `${quadraticaCount}/14`;
  document.getElementById(
    "exponencialCount"
  ).textContent = `${exponencialCount}/11`;
  document.getElementById(
    "logaritmicaCount"
  ).textContent = `${logaritmicaCount}/11`;
}

function checkWin() {
  if (
    afimCount + quadraticaCount + exponencialCount + logaritmicaCount ===
    50
  ) {
    setTimeout(() => {
      alert(
        "Parabéns, você finalizou o jogo! Caso queira jogar novamente, clique no botão RESET."
      );
    }, 100);
  }
}
